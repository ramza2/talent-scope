"""Detailed profile extraction prompt — version profile-extract-v3.

Based on v2 provenance rules, with tighter career_document_value semantics:
short total-career expressions only (max 100 chars), else null.
"""

from __future__ import annotations

PROMPT_VERSION = "profile-extract-v3"
  ...
  "projects": [
    {
      ...
      "jobs": [{"raw_value": null, "code": null, "source_refs": []}],
      "skills": [{"raw_value": null, "code": null, "source_refs": []}],
      "expertise": [{"raw_value": null, "code": null, "source_refs": []}],
      "business_domains": [{"raw_value": null, "code": null, "source_refs": []}],
      "customer_types": [{"raw_value": null, "code": null, "source_refs": []}],
      "confidence": null,
      "source_refs": []
    }
  ],
...
- profile.source_refs는 필드별 근거입니다. 문서에 해당 필드 값이 명시된 경우에만
  해당 키에 quote를 넣습니다. 억지로 모든 필드에 Evidence를 만들지 않습니다.
- Project relation(jobs/skills/expertise/business_domains/customer_types)의
  source_refs는 해당 JOB/TECH/EXP/BIZ/CUSTOMER_TYPE 자체를 뒷받침하는 quote만 사용합니다.
  Project 전체를 설명하는 일반 quote를 개별 relation 근거로 복사하지 마십시오.
  Project 일반 설명은 projects[].source_refs에만 둡니다.
...
def build_user_prompt(
    *,
    ...
    return (
        ...
        "profile.source_refs와 Project relation item source_refs를 구분하세요.\n"
        "JSON object만 반환하세요.\n\n"
        "===== BEGIN CODE CATALOG =====\n"
        f"{code_catalog}\n"
        "===== END CODE CATALOG =====\n\n"
