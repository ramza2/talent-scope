...
구현 작업 전 현재 요청과 직접 관련된 설계문서만 확인한다.
작업과 무관한 문서를 넓게 읽지 말고, 명시된 파일/심볼/테스트부터 시작해 필요한 경우에만 탐색 범위를 확장한다.
...
- 무관한 리팩터링, rename, dependency 추가, TODO 정리 등을 함께 하지 않는다.
- 문제를 발견해도 현재 요청과 직접 관련 없으면 자동 수정하지 말고 결과에 짧게 보고한다.
- 기존 구현 패턴과 인터페이스를 우선 재사용하고, 새 구조는 필요한 경우에만 만든다.
- 사용자가 이미 확인한 사실, 성공한 테스트, 확정된 모델/Endpoint/설정은 변경 정황이 없으면 재조사하지 않는다.

다음 단계 구현은 사용자의 별도 요청 후 진행한다.

## 15. Efficient Repository Exploration

전체 저장소를 기본값으로 탐색하지 않는다.

탐색 순서:

1. 사용자가 지정한 파일/모듈
2. 해당 코드가 직접 호출/import하는 코드
3. 같은 기능의 기존 테스트
4. 원인 파악에 필요한 최소 인접 영역

검색 시에는 가능한 한 정확한 심볼, Endpoint, 오류문구, 테이블명, 함수명을 사용한다.
관련 실행경로를 확인했으면 불필요한 탐색을 중단한다.
...
1. Focused Test
   - 변경한 기능과 직접 관련된 테스트만 우선 실행한다.
2. Impacted-area Test
   - 공용 로직을 변경했을 때만 관련 모듈 범위로 넓힌다.
3. Full Regression
   - 사용자가 명시적으로 요청했거나 릴리스/공용 인프라 변경 등 꼭 필요한 경우에만 실행한다.

추가 원칙:

- 코드 변경 없이 이미 성공한 테스트를 반복 실행하지 않는다.
- 작은 수정마다 전체 build/pytest/CI를 반복하지 않는다.
- 서버 전체 회귀를 사용자가 직접 실행하는 흐름이면 Agent가 중복 실행하지 않는다.
...
### Notes
- blocker, risk, 설계 불확실성, 범위 밖에서 발견한 중요한 문제만 기재
